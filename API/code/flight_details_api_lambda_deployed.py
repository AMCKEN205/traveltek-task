...
def lambda_handler(event, context):
    ...
    # Will all be GET requests so no need to handle other http methods (POST etc.)
    requested_data = event["queryStringParameters"]["requestedData"]
    
    # Call data retrieval function based on the resource we want 
    
    data_to_return = None 
    
    # No support for switch/case in python so have to use if else if chain
    try:
        if requested_data == "total-flights-count":
            data_to_return = get_all_flights_count()
        
        elif requested_data == "morning-flights-count":
            data_to_return = get_morning_flights_count()
        
        elif requested_data == "sweden-flights-data":
            data_to_return = get_sweden_flight_data()
            
        elif requested_data == "top10-destinations":
            data_to_return = get_top10_destinations()
            
        elif requested_data == "average-journey-times":
            data_to_return = get_average_journey_times()
        
        elif requested_data == "flight-class-appearances":
            data_to_return = get_flight_class_appearances()
